...
function Card() {
  ...
  return (
    <div className='w-full h-34 grid grid-cols-6 '>
        {imges.map((obj,index)=>(
        <div key={index} className='h-full w-55 bg-cover bg-no-repeat  relative bg-center group' style={{ backgroundImage: `url(${obj.bg})` }}>
        ...
        <img src={obj.logo} className=' absolute top-12 left-8' alt="" />
        </div>
        ))}
    </div>
  )
